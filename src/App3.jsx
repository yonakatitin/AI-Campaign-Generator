...
const BtnOutline = ({ onClick, disabled, children }) => (
  <button onClick={onClick} disabled={disabled} style={{ padding: "10px 22px", background: C.white, color: C.dark, border: `1px solid ${C.border}`, borderRadius: 6, fontWeight: 400, fontSize: 13, cursor: disabled ? "not-allowed" : "pointer", fontFamily: F }}>
    {children}
  </button>
);
...
const BMCForm = ({ onSubmit, loading }) => {
  ...
  return (
    <div style={{ background: C.white, borderRadius: 8, border: `1px solid ${C.border}`, width: "100%", boxSizing: "border-box" }}>
      {/* Form Header */}
...
const ReviewField = ({ f, v, onChange }) => {
  const isArray = Array.isArray(v);
  const isEmpty = !v || (typeof v === "string" && (v.trim() === "" || v.trim() === "-")) || (isArray && v.length === 0);
  ...
  return (
    <div>
      ...
      {f.array && isArray ? (
        ...
        <div style={{ background: isEmpty ? C.cream : C.white, border: `1px solid ${C.border}`, borderRadius: 4, padding: "8px 10px" }}>
          {v.length > 0
            ? v.map((item, i) => (
                <div key={i} style={{ display: "flex", gap: 8, alignItems: "flex-start", marginBottom: i < v.length - 1 ? 6 : 0 }}>
                  <span style={{ color: C.orange, fontWeight: 700, flexShrink: 0, fontFamily: F }}>·</span>
                  <span style={{ fontSize: 13, color: C.dark, lineHeight: 1.5, fontFamily: F }}>{item}</span>
                </div>
              ))
            : <span style={{ fontSize: 13, color: C.light, fontFamily: F }}>—</span>}
        </div>
      ) : f.area ? (
      ...
      <div style={{ padding: "28px 32px" }}>
        {REVIEW_SECTIONS.map((section, si) => {
          ...
          return (
            <div key={section.section}>
              {si > 0 && <Divider />}
              <SectionTitle children={section.section} sub={section.section === "Campaign Intelligence" ? "Generated by AI" : null} />
              <div style={{ display: "grid", gridTemplateColumns: section.section === "Campaign Info" ? "1fr 1fr" : "1fr", gap: 16 }}>
                {visibleFields.map((f) => (
...
const CopyBtn = ({ text, id, copied, copy }) => (
  <button onClick={() => copy(text, id)} style={{ fontSize: 11, color: copied === id ? C.orange : C.light, background: "none", border: "none", cursor: "pointer", fontWeight: 500, fontFamily: F, padding: 0 }}>
...
const CampaignOutput = ({ campaign, qualityCheck, meta, extracted, onReset, onRegenerate, regenerating }) => {
  ...
  return (
    <div>
      {/* Score banner */}
      <div style={{ background: C.black, borderRadius: 8, padding: "20px 28px", marginBottom: 16, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
          <div style={{ fontSize: 11, color: "#666", textTransform: "uppercase", letterSpacing: "0.12em", marginBottom: 6, fontFamily: F }}>Quality Score</div>
          <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
            <span style={{ fontSize: 36, fontWeight: 700, color: C.white, fontFamily: F }}>{score}</span>
            <span style={{ fontSize: 14, color: "#666", fontFamily: F }}>/100</span>
          </div>
          <div style={{ marginTop: 10, width: 180, height: 3, background: "#333", borderRadius: 2 }}>
            <div style={{ width: `${score}%`, height: "100%", background: score >= 80 ? C.orange : "#888", borderRadius: 2, transition: "width 1s ease" }} />
          </div>
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: 4, textAlign: "right" }}>
          {issues.length === 0 && <span style={{ fontSize: 12, color: C.orange, fontFamily: F }}>✓ No critical issues</span>}
          {issues.map((x, i) => <span key={i} style={{ fontSize: 12, color: "#FF8060", fontFamily: F }}>⚠ {x}</span>)}
          {suggestions.map((x, i) => <span key={i} style={{ fontSize: 12, color: "#FFD080", fontFamily: F }}>→ {x}</span>)}
        </div>
      </div>
      ...
      <div style={{ background: C.white, borderRadius: 8, border: `1px solid ${C.border}` }}>
        ...
        <div style={{ display: "flex", borderBottom: `1px solid ${C.border}`, padding: "0 32px" }}>
          {TABS.map((t, i) => (
            <button key={i} onClick={() => setActiveTab(i)} style={{ padding: "16px 0", marginRight: 28, background: "none", border: "none", borderBottom: `2px solid ${activeTab === i ? C.orange : "transparent"}`, color: activeTab === i ? C.dark : C.light, fontWeight: activeTab === i ? 500 : 400, fontSize: 13, cursor: "pointer", fontFamily: F, transition: "all 0.15s" }}>
              {t}
            </button>
          ))}
        </div>
          ...
          {activeTab === 0 && (
            <div>
              ...
              <ContentBlock label="Subheadline" copyText={lp.subheadline} id="lp-s" copied={copied} copy={copy}>
                <div style={{ fontSize: 15, color: C.mid, lineHeight: 1.65, fontFamily: F, fontWeight: 300 }}>{lp.subheadline}</div>
              </ContentBlock>
          ...
          {activeTab === 1 && (
            <div style={{ display: "grid", gap: 12 }}>
              {Object.values(linkedin).map((phase, i) => (
                <div key={i} style={{ border: `1px solid ${C.border}`, borderRadius: 6, padding: "20px 24px", background: i === 0 ? "#FFF8F0" : C.white }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
                    <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                      <div style={{ width: 6, height: 6, borderRadius: "50%", background: i === 0 ? C.orange : C.mid, flexShrink: 0, marginTop: 1 }} />
                      <span style={{ fontSize: 11, fontWeight: 500, color: i === 0 ? C.orange : C.mid, textTransform: "uppercase", letterSpacing: "0.1em", fontFamily: F }}>{phase.label}</span>
                    </div>
        ...
        <div style={{ padding: "16px 32px", display: "flex", justifyContent: "space-between", alignItems: "center", background: C.surface, borderRadius: "0 0 8px 8px" }}>
          <div style={{ display: "flex", gap: 8 }}>
            <BtnOutline onClick={exportAll}>{copied === "all" ? "Copied ✓" : "Copy all"}</BtnOutline>
            <BtnOutline onClick={() => downloadWord(campaign, extracted)}>Download Word</BtnOutline>
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <BtnOutline onClick={onRegenerate} disabled={regenerating}>{regenerating ? "Regenerating..." : "Regenerate"}</BtnOutline>
            <BtnPrimary onClick={onReset}>New Campaign ↗</BtnPrimary>
          </div>
        </div>
      </div>
