          ...
          {activeTab === 2 && (
            <div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 24 }}>
                {[{ label: "Subject Line", key: "subject_line" }, { label: "Preview Text", key: "preview_text" }].map((f) => (
                  <div key={f.key} style={{ padding: "14px 16px", background: C.surface, borderRadius: 6, border: `1px solid ${C.border}` }}>
                    <div style={{ fontSize: 11, color: C.light, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 6, fontFamily: F }}>{f.label}</div>
                    <div style={{ fontSize: 13, fontWeight: 500, color: C.dark, fontFamily: F }}>{email[f.key]}</div>
                  </div>
                ))}
              </div>
              <ContentBlock label="Email Body" copyText={email.body} id="email-body" copied={copied} copy={copy}>
                <div style={{ fontSize: 14, color: C.dark, lineHeight: 1.8, whiteSpace: "pre-line", fontFamily: F, padding: "16px", background: C.surface, borderRadius: 6 }}>{email.body}</div>
              </ContentBlock>
              <ContentBlock label="CTA Button">
                <div style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "10px 22px", background: C.orange, color: C.white, borderRadius: 6, fontWeight: 500, fontSize: 14, fontFamily: F }}>
                  {email.cta_text} ↗
